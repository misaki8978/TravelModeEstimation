...
#バスのデータフレームとファイルを取得
# bus_gdf, file, year, place = division_several_file(files) #geopandasのデータフレームに変換

# OUT_PROCESSED = f"/home/data/fukui/processed/09_04_{place}/{year}"
# os.makedirs(OUT_PROCESSED+"/GPS", exist_ok=True)

# #鉄道ネットワークを取得
# target_place = "Nagasaki, Japan"
# train_gdf = train_from_OSM(target_place)

# #バッファーを作成
# train_buffer = make_buffer(train_gdf, 100)
...
log_message(f"{gdf_features_walk.columns}", log_path)
...
log_message(f"{gdf_walk_nonwalk['label'].value_counts()}", log_path)