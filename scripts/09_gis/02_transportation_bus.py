...
def read_results_concat(result_files: list[str]) -> pd.DataFrame:
    ...
    for f in result_files:
        try:
            df = pd.read_csv(f)
            for dt_col in ["segment_start", "segment_end"]:
...
def filter_segments(
    ...
    mode: str,
    route_threshold: float = 0.9,
    min_points: int = 10,
) -> pd.DataFrame:
    if mode == "rail":
        raise ValueError("rail mode is not supported in this script")
    elif mode == "bus":
        mask = df_results["point_ratio_in_bus_route_buffer"] >= route_threshold
        if "num_points" in df_results.columns:
            mask &= df_results["num_points"] >= min_points
    else:
        raise ValueError("mode must be 'rail' or 'bus'")
    cols = ["hashed_adid", "segment_id"]
    for c in ["segment_start", "segment_end", "segment_date"]:
...
def plot_segments(gps_df: pd.DataFrame, keep_pairs: pd.DataFrame, out_path: str, title: str, color: str) -> None:
    ...
    if not gps_keep.empty:
        if {"segment_start", "segment_end"}.issubset(gps_keep.columns):
            mask_time = (gps_keep["datetime"] >= gps_keep["segment_start"]) & (gps_keep["datetime"] <= gps_keep["segment_end"])
            gps_keep = gps_keep.loc[mask_time]
        elif "segment_date" in gps_keep.columns:
            gps_keep["_date"] = gps_keep["datetime"].dt.date.astype(str)
            gps_keep = gps_keep.loc[gps_keep["_date"] == gps_keep["segment_date"]].drop(columns=["_date"]) 
    if gps_keep.empty:
...
def plot_segment_all(gps_df: pd.DataFrame, layer_df: pd.DataFrame, out_path: str, title: str, color: str) -> None:
    if gps_df.empty or layer_df.empty:
        print(f"No data to plot for {title}")
        return
    ...
    layer_gdf = gpd.GeoDataFrame(
        layer_df,
        geometry=gpd.points_from_xy(layer_df["longitude_anonymous"], layer_df["latitude_anonymous"]),
        crs="EPSG:4326",
    )
    fig, ax = plt.subplots(figsize=(10, 10))
    gdf.plot(ax=ax, color=color, markersize=2, alpha=0.7)
    layer_gdf.plot(ax=ax, color=color, markersize=2, alpha=0.7)
    ax.set_title(title)
    ax.set_axis_off()
    plt.tight_layout()
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
...
def main():
    ...
    files = sys.argv[1:]
    if "--" not in files:
        raise ValueError("Usage: python 02_transportation_bus.py <gps_csv_gz ...> -- <trip_results_csv ...>")
    ...
    for month in months:
        ...
        required_res_cols = {"hashed_adid", "segment_id", "point_ratio_in_bus_route_buffer"}
    ...
    bus_out_merged = os.path.join(out_dir, "merged_bus_segments_test.png")

    plot_segments(
        gps_df=gps_all,
        keep_pairs=keep_bus_all,
        out_path=bus_out_merged,
        title=f"{place} {year} Bus-like segments merged (route>=0.6)",
        color="blue",
    )

    log_message("Finished 02_transportation_bus", log_path)
