...
# total_distance_sorted = combined_stays['move_total_distance'].sort_values(ascending=False).reset_index(drop=True)
...
# move_duration_min_sorted = combined_stays['move_total_duration_sec'].sort_values(ascending=False).reset_index(drop=True)
...
# fig.delaxes(axs[5])
# #.png出力
# plt.savefig(f'{OUT_DIR}/02_{year}_{place}_replay.png')
