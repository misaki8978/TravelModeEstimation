...
def calculate_speed_and_acceleration(df):
    speed_df = df.assign(
                        ...
                        distance=lambda x: haversine_distance(x['prev_lat'], x['prev_lon'], x['latitude_anonymous'], x['longitude_anonymous']),
                        speed=lambda x: x['distance']/x['time_diff_s'],
                        acceleration=lambda x: x['speed'].diff(1)/x['time_diff_s'],
                        )\
                .drop(columns=["prev_lat", "prev_lon", "time_diff_s"])
    return speed_df

def segment_by_label(df, v_thd, a_thd):

    speed_df = calculate_speed_and_acceleration(df)
    speed_df = speed_df.assign(
                                label=lambda x: np.where((x['speed'] < v_thd) & (x['acceleration'] < a_thd),\
                                'walk', 'non-walk')
                                )
    
    segments = []
    current_label = speed_df.iloc[0]['label']
    current_segment = [speed_df.iloc[0]]
...
def process_all_segments(df, v_thd, a_thd, dist_thd1, dist_thd2, limit_thd):
    ...
    for move_id, group in df.groupby('move_id'):
        # speed_df = calculate_speed_and_acceleration(group)
    ...
    results = pd.concat(all_results)\
                ...
                .reset_index(drop=True)
    final_df = results[["hashed_adid", "datetime", "move_id", "segment_id", "label", "label_cer",  "speed", "acceleration", "latitude_anonymous", "longitude_anonymous", "accuracy"]]
    return final_df