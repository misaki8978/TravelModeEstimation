...
def worker(task_id):
    ...
    if func_name_global == "stayPointExtraction":
        if target_id not in base_dict:
            return pd.DataFrame(columns=["hashed_adid","week_start","latitude","longitude","datetime"])
        df = base_dict[target_id]
        # log_message(f"df: {df.head()}", message_path)
        points = [
                    Point(row['latitude_anonymous'], row['longitude_anonymous'], row['datetime'], row['hashed_adid'], row['week_start'])
                    for _, row in df.iterrows()
                ]
        # log_message(f"stayPointExtraction: {month_global} {target_id}", message_path)
        return_df = stayPointExtraction(points, roam_dist, stay_dur)
        # log_message(f"return_df: {return_df.head()}", message_path)
        return return_df
...
if __name__ == "__main__":
    ...
    all_files = []
    for file in files:
        df_1file = pd.read_csv(file, parse_dates=["datetime"], compression="gzip") \
                .query("accuracy <= 1000")\
                .assign(
        ...
        all_files.append(df_1file)
    df = pd.concat(all_files, ignore_index=True)
    # log_message(f"{df.head()}", message_path)
    base_dict = {k: v for k, v in df.groupby(["hashed_adid", "week_start"])}
    # log_message(f"{month} base_dict: {len(base_dict)}", message_path)
    ...
    for func_name in funcs:
        out_path = os.path.join(OUT_DIR, f"{func_name}_{month}_{task_id}.csv")

        if os.path.exists(out_path):
            log_message(f"[SKIP] {month} {func_name}", message_path)
            results_df = pd.read_csv(out_path)
            results_prev_dict = {k: v for k, v in results_df.groupby(["hashed_adid", "week_start"])}
            continue
