...
unique_segments = mode_gps_df[['unique_key', 'mode_label']].drop_duplicates()

# 各クラスタからランダムに指定数だけセグメントIDを抽出
sampled_segments_df = unique_segments.groupby('mode_label', group_keys=False).apply(
...
sampled_segments_df.to_csv(f'{OUT_DIR}/ground_truth_labeling_sheet.csv', index=False)
df_sampled.to_csv(f'{OUT_DIR}/sampled_mode_gps.csv', index=False)
